...
const { onConnect, onNodesChange, onEdgesChange, project, addEdges } = useVueFlow()

onMounted(() => {
  if (props.initialCanvas) {
    nodes.value = props.initialCanvas.nodes || []
    edges.value = props.initialCanvas.edges || []
  }
})

watch(() => props.initialCanvas, (newCanvas: WorkflowCanvas | undefined) => {
  if (newCanvas) {
    nodes.value = newCanvas.nodes || []
    edges.value = newCanvas.edges || []
    isDirty.value = false
  }
}, { deep: true })
...
function onNodeClick(event: NodeMouseEvent) {
  selectedNode.value = event.node as WorkflowNode
}
...
defineExpose({
  getCanvas: () => ({ nodes: nodes.value, edges: edges.value }),
